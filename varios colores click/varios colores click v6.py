...
grid = 5
rad = 30
ladox = 70 #cambiar para que abarque donde se para el carro
ladoy = 50
meta = (-1,-1)
vladi = (-1,-1)
vladi2 = (-1,-1)
...
#Transformacion trigonometrica para la pista
...
def click(event, x, y, flags, param):
    global px, hue, pista, cruces, remove, meta
...
while(1):
    ...
    if(HSV):
        ...
        #Al presionar la tecla space se guardan los valores en la lista filtros
        if k == 32: #tecla space
    ...
    #Si se estan escogiendo colores se usa la mask de prueba            
    elif(HSV):
    ...
    if not cruces and not HSV:
        ...
        for cr in pxcruces:
            ...
            for fil in filtros[1:]:
                if (fil.centro[0] > cr.centro[0] - ladox and fil.centro[0] < cr.centro[0] + ladox and
                    fil.centro[1] > cr.centro[1] - ladoy and fil.centro[1] < cr.centro[1] + ladoy):
                    obstaculos.append(cr.pos)

    #si no se esta en un cruce se manda 255
    if entrecruce and not HSV:
        enviar(255)
        ...
        for fil in filtros:
            if fil.vladi:#azul, vladi
                imagen = cv2.rectangle(imagen,((fil.centro[0]/grid)*grid,(fil.centro[1]/grid)*grid),
                                       ...
                                       (255,0,0),-1)
            elif fil.danger and not fil.vladi:#rojo, si esta muy cerca
                imagen = cv2.rectangle(imagen,((fil.centro[0]/grid)*grid,(fil.centro[1]/grid)*grid),
                                       ((fil.centro[0]/grid+1)*grid,(fil.centro[1]/grid+1)*grid),
                                       (0,0,255),-1)
            else:#verde, si esta lejos
                imagen = cv2.rectangle(imagen,((fil.centro[0]/grid)*grid,(fil.centro[1]/grid)*grid),
                                       ((fil.centro[0]/grid+1)*grid,(fil.centro[1]/grid+1)*grid),
                                       (0,255,0),-1)
        ...
        if meta != (-1,-1) and vladi != (-1,-1):
            ...
            if vladi != vladi2 or para:
                ...
                for fil in filtros[1:]:
                    ...
                    if (fil.centro[0] > px1[0] + 170 - ladox/2 and fil.centro[0] < px1[0] + 170 + ladoy/2
                        and fil.centro[1] > px1[1] - ladoy/2 and fil.centro[1] < px1[1] + ladoy/2):
                        print 'Obstaculo a la derecha'
                        der = True

                    if (fil.centro[0] > px1[0] - 170 - ladox/2 and fil.centro[0] < px1[0] - 170 + ladoy/2
                          and fil.centro[1] > px1[1] - 60 and fil.centro[1] < px1[1] + 60):
                        print 'Obstaculo a la izquierda'
                        izq = True

                    #150, 90
                    if (fil.centro[0] > px1[0] - 85 and fil.centro[0] < px1[0] + 85
                          and fil.centro[1] > px1[1] - 120 - ladoy/2 and fil.centro[1] < px1[1] - 120 + ladoy/2):
    ...
    cv2.imshow('Camara', imagen)#poner la imagen mas grande
